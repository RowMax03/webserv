...
namespace Config {
    Location::Location() : path(""), redirect_status(0), redirect_url(""), root(""), autoindex(false), index("") {}

    Location::Location(const Location &other) : confBase(other), path(other.path),
    ...
    void Location::setConfigValue(const std::vector <std::string> &configValues) {
