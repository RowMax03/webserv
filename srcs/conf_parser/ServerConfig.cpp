...
namespace Config {
    Server::Server() : listen(0), server_name(""), server_names_hash_bucket_size(0), client_max_body_size("") {
        locations = std::map<std::string, Location>();
    }

    Server::Server(const Server &other) : confBase(other), listen(other.listen), server_name(other.server_name),
    ...
    void Server::setConfigValue(const std::vector <std::string> &configValues) {
